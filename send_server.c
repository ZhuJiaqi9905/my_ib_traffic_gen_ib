...
static bool recv_traffic(struct conn_context *connections, unsigned int num_qps,
                         unsigned int iters, bool use_event);
...
static void print_usage(char *app) {
  ...
  fprintf(stderr,
          ...
          DEFAULT_NUM_QPS);
  fprintf(stderr,
          "  -r  --rx-depth=<dep>      size of rx queue of each queue pair "
          "(default %d)\n",
          DEFAULT_RX_DEPTH);
